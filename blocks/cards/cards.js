...
function setupCard(row) {
  [...row.children].forEach((div) => {
    if (div.children.length === 1 && div.querySelector('picture')) div.className = 'cards-card-image';
      ...
      if (title) {
        const subtitle = title.querySelector('p > strong');
...
export default async function decorate(block) {
  ...
  [...block.children].forEach((row) => {
    ...
    // wrap main card element in an anchor if card has button
    const button = row.querySelector('.button');
    if (button && button.href) {
      const title = row.querySelector('h2');
      const a = document.createElement('a');
      a.href = button.href;
      a.title = title ? title.textContent : button.title;
      a.setAttribute('aria-label', title ? title.textContent : button.title);
      // hide duplicate link from screenreaders
